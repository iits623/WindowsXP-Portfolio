    ...
    <div v-for="window in windows" :key="window.id">
      ...
      >
        <component
          :is="window.component"
          v-bind="
            window.leftMenuType ? { leftMenuType: window.leftMenuType } : {}
          "
        />
      </Window>
    </div>
...
const closeWindow = (windowId) => {
  const windowIndex = windows.value.findIndex(
...
const isWindowVisible = (windowId) =>
  windows.value.some((window) => window.id === windowId);
