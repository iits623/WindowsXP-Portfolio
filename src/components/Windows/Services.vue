<template>
  <div class="relative right-0 h-content-window flex">
    ...
    <div
      v-if="goBackStore.currentActiveService"
      ...
      <div class="m-4">
        <div class="w-full gap-4 mb-4">
          <div class="flex items-center gap-3 mb-2">
            <img
              :src="getIconPath(goBackStore.currentActiveService.icon)"
              alt="service icon"
              class="w-12 h-12"
            />
            <h2 class="text-xl md:text-2xl font-semibold">
              {{ getLocalizedServiceName(goBackStore.currentActiveService) }}
            </h2>
          </div>
          ...
          <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
            <p class="text-xs md:text-sm text-gray-700">
              {{
                goBackStore.currentActiveService.description[
                  localeStore.currentLocale
                ] || goBackStore.currentActiveService.description["fr"]
              }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div class="bg-gray-50 p-4 rounded">
              <h3 class="text-sm font-semibold text-gray-600 mb-2">
                {{ $t("windows.services.price") }}
              </h3>
              <p class="text-2xl font-bold text-blue-600">
                €{{ goBackStore.currentActiveService.price }}
              </p>
            </div>

            <div class="bg-gray-50 p-4 rounded">
              <h3 class="text-sm font-semibold text-gray-600 mb-2">
                {{ $t("windows.services.duration") }}
              </h3>
              <p class="text-lg font-semibold text-gray-800">
                {{
                  goBackStore.currentActiveService.duration[
                    localeStore.currentLocale
                  ] || goBackStore.currentActiveService.duration["fr"]
                }}
              </p>
            </div>
          </div>
          ...
          <div v-if="goBackStore.currentActiveService.features" class="mb-4">
            <h3 class="text-lg font-semibold mb-3">
              {{ $t("windows.services.features") }}
            </h3>
            <ul class="list-disc list-inside space-y-2">
              <li
                v-for="(feature, index) in goBackStore.currentActiveService
                  .features[localeStore.currentLocale] ||
                goBackStore.currentActiveService.features['fr']"
                :key="index"
          ...
          <div class="mt-6 border-t-2 border-gray-200 pt-4">
            <div
              class="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-lg"
            >
              ...
              <div class="flex flex-col sm:flex-row gap-3">
                ...
                >
                  ...
                  <span class="text-xs font-medium">{{
                    $t("windows.services.contactForm")
                  }}</span>
                </button>
              </div>
            </div>
        ...
        <div class="flex flex-wrap gap-2 pt-2 md:pt-3 pb-3 w-full">
          ...
          >
            <img
              ...
              alt="service"
              class="w-10 h-10"
              :style="{ opacity: service.isFocus ? 0.5 : 1 }"
            />
            <p
...
const emailAddress =
  import.meta.env.VITE_APP_ADMIN_EMAIL_ADDRESS || "default@example.com";
...
const getIconPath = (icon) => {
  if (!icon) return "";
  return `${import.meta.env.BASE_URL}img/icons/${icon}`;
};
</script>
