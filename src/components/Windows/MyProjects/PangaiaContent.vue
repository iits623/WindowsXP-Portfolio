<template>
  <div class="text-xs md:text-header-window">
    <div class="flex text-xxs md:text-xs w-full items-center justify-center md:justify-start flex-wrap gap-4 md:min-w-80 md:ml-1">
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/vuejs.svg`" :alt="$t('common.icon') + ' vuejs'" class="w-9 h-9 p-px" />
      ...
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/mariadb.svg`" :alt="$t('common.icon') + ' mariadb'" class="w-9 h-9" />
        <p class="font-bold mt-px">Maria DB</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/swagger.svg`" :alt="$t('common.icon') + ' swagger'" class="w-9 h-9" />
        <p class="font-bold mt-px">Swagger</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/postman.svg`" :alt="$t('common.icon') + ' postman'" class="w-9 h-9" />
        <p class="font-bold mt-0.5">Postman</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/workbench.svg`" :alt="$t('common.icon') + ' workbench'" class="w-9 h-9" />
        <p class="font-bold">Workbench</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/looping.svg`" :alt="$t('common.icon') + ' looping'" class="w-10 h-10" />
        <p class="font-bold">Looping</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="`${$baseUrl}img/icons/projects/tools/docker.svg`" :alt="$t('common.icon') + ' docker'" class="w-9 h-9" />
    ...
    <div class="mb-2">
      ...
      <img :src="`${$baseUrl}img/projects/pangaia/pangaia-database-MCD.webp`" :alt="$t('common.CDM') + ' - Pangaia'" class="w-full max-w-[750px]" />
    </div>
    <div>
      ...
      <img :src="`${$baseUrl}img/projects/pangaia/MPD-Pangaia.svg`" :alt="$t('common.PDM') + ' - Pangaia'" class="w-full" />
    </div>
  </div>
  <a
    rel="noopener"
  ...
  >
    <img :src="`${$baseUrl}img/icons/projects/tools/web.svg`" :alt="$t('buttons.linkWebSite') + ' ' +  ProjectName" class="w-3 h-3 mr-0.5"/>
