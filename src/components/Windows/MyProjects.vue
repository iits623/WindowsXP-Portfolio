...
const handleLeftMenuItemClick = (categoryName) => {
  ...
  const category = categories.value.find(
    (c) =>
      getLocalizedCategoryName(c) === categoryName ||
      c.name.en === categoryName ||
      c.name.fa === categoryName,
  );
...
const currentCategoryProjects = computed(() => {
  ...
  const category = categories.value.find(
    (c) =>
      getLocalizedCategoryName(c) === selectedCategoryName.value ||
      c.name.en === selectedCategoryName.value ||
      c.name.fa === selectedCategoryName.value,
  );
...
const localizedEmptyTitle = computed(() => {
  return localeStore.currentLocale === "fa"
    ? "این پوشه خالی است"
...
const localizedEmptySubtitle = computed(() => {
  return localeStore.currentLocale === "fa"
    ? "از منوی سمت چپ یک آیتم را انتخاب کنید"
    : "Select an item from the left menu";
});

const getLocalizedTitle = (project) => {
  return (
    project?.title?.[localeStore.currentLocale] ||
    project?.title?.["en"] ||
    project?.name ||
    "بدون عنوان"
  );
...
<template>
  <div class="relative right-0 h-content-window flex">
    <WindowLeftMenu
      :leftMenuType="props.leftMenuType"
      @item-click="handleLeftMenuItemClick"
    />

    <div
      v-if="selectedCategoryName && currentCategoryProjects.length > 0"
    ...
    <div
      v-else
      class="w-full h-full bg-[#d4d0c8] flex items-center justify-center"
    >
      <div class="text-center">
        <img
          :src="`${$baseUrl}img/icons/folder-icon.webp`"
          class="w-20 h-20 mx-auto opacity-50"
        />
        <p class="text-sm text-gray-600 font-tahoma mt-3">
          {{ localizedEmptyTitle }}
        </p>
        <p class="text-xs text-gray-400 font-tahoma">
          {{ localizedEmptySubtitle }}
        </p>
      </div>
    </div>
  </div>
